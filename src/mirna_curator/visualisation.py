...
def create_miRNA_flowchart_viz(recorded_df: pl.DataFrame, expected_df: pl.DataFrame, filter_class: int):
    """
    ...
    recorded_df = recorded_df.with_columns([pl.col(col).cast(pl.Boolean) for col in node_columns])
    recorded_df = recorded_df.with_columns([pl.col(f"{col}_result").cast(pl.Boolean) for col in filter(lambda x: not x[0] in ['v', 'n'], node_columns)])

    expected_df = expected_df.with_columns([pl.col(col).cast(pl.Boolean) for col in node_columns])
    expected_df = expected_df.with_columns([pl.col(f"{col}_result").cast(pl.Boolean) for col in filter(lambda x: not x[0] in ['v', 'n'], node_columns)])
    
    expected_columns = expected_df.columns
    expected_to_count = expected_df.join(recorded_df, on="PMCID", how='inner').select(expected_columns).unique("PMCID")
    
    expected_node_count = get_edges_count(expected_to_count, cf)
    recorded_columns = recorded_df.columns
    recorded_to_count = recorded_df.join(expected_df, on="PMCID", how='inner').select(recorded_columns).unique("PMCID")
    ...
    for edge in edges:
        ...
        edges_labels[edge] = f"Recorded: {recorded_edge_count}\nExpected: {expected_edge_count}"
    ...
    for node, (x, y) in pos.items():
        plt.text(x, y, node_labels[node], horizontalalignment='center', wrap=True, bbox=dict(facecolor='white', alpha=0.5, boxstyle='round'))
    # Draw edges
    nx.draw_networkx_edges(G, pos, edge_color=edge_colours, arrows=True,
                          arrowsize=20)
...
@click.option("--filter_class", default=None, help="Filter to one class, None means no filtering", type=int)
